...
<section class="plugin-pages page-edit">
	...
	<?=form_helper::openForm()?>
		...
		<fieldset class="form break <?=text_helper::alternate()?>">
			...
			<? foreach ( $fields as $field ): ?>
				...
				<? else: ?>
					...
					<div class="row <?=text_helper::alternate('odd','even')?>" id="input_row_page_<?=( isset($field['system']) ? 'data_' : '' )?><?=$field['keyword']?>">

						<label for="input_edit_page_<?=( isset($field['system']) ? 'data_' : '' )?><?=$field['keyword']?>">
							...
							<div class="field">
								...
								<? view::load('system/elements/field/edit', array(
									...
									'field' => array(
										'keyword' => 'keyword',
										'type' => 'text',
										'maxlength' => isset($field['config']['max_length']) ? $field['config']['max_length'] : 255,
									),
									'value' => array('keyword' => isset($page['keyword']) ? $page['keyword'] : ''),
								)) ?>
		...
		<fieldset class="form grid <?=text_helper::alternate()?>">
			...
			<? foreach ( $options as $option ): ?>
				...
				<div class="row <?=text_helper::alternate('odd','even')?>" id="input_row_page_<?=$option['keyword']?>">

					<label for="input_edit_page_<?=$option['keyword']?>">
					...
					<div class="field">
						...
						<? view::load('system/elements/field/edit', array(
							'prefix' => 'page',
							'field' => $option,
							'value' => isset($page[$option['keyword']]) ? $page : $option,
						)) ?>
	...
	<?=form_helper::closeForm(array('do_save_page' => 1))?>
