	...
	<div class="content-view">
		...
		<article class="item clearfix">
			...
			<figure class="image users-image source">
				<img src="<?=storage_helper::getFileURL(session::item('picture', 'signup', 'service_id'), session::item('picture', 'signup', 'path'), session::item('picture', 'signup', 'name'), session::item('picture', 'signup', 'extension'))?>" style="max-width:400px;max-height:400px;" />
			</figure>

			<figure class="image users-image preview">
				<img src="<?=storage_helper::getFileURL(session::item('picture', 'signup', 'service_id'), session::item('picture', 'signup', 'path'), session::item('picture', 'signup', 'name'), session::item('picture', 'signup', 'extension'))?>" />
			</figure>
		...
		<?=form_helper::openForm()?>
			...
			<fieldset class="form">
				...
				<div class="row actions">
					...
					&nbsp;
					<?=html_helper::anchor('users/signup/picture', __('cancel', 'system'))?>
				</div>
...
function createCropArea()
{
	...
	$('.profile-picture-thumbnail figure.source img').Jcrop({
		...
		trueSize: [<?=session::item('picture', 'signup', 'width')?>,<?=session::item('picture', 'signup', 'height')?>],
