					...
					<article class="item">
						...
						<footer class="item-footer">
							...
							<ul class="unstyled content-meta clearfix">
								<li class="date">
									<?=__('author_date', 'system_info', array('%author' => users_helper::anchor($blog['user']), '%date' => date_helper::formatDate($blog['post_date'])))?>
								</li>
								...
								<? if ( $blog['user_id'] == session::item('user_id') ): ?>
									<li class="actions">
										<?=html_helper::anchor('blogs/edit/'.$blog['blog_id'], __('blog_edit', 'blogs'))?>
										<?=html_helper::anchor('blogs/delete/'.$blog['blog_id'].'?'.$qstring['url'].'page='.$qstring['page'], __('blog_delete', 'blogs'), array('data-html' => __('blog_delete?', 'blogs'), 'data-role' => 'confirm'))?>
									</li>
								<? endif; ?>
